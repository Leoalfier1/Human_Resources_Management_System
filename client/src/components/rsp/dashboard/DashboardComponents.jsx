...
export const TATCard = ({ items, target }) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col h-full">
    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Turnaround Time (TAT)</h4>
...
export const DeadlinesCard = ({ items }) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col h-full">
    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Upcoming Deadlines</h4>
        ...
        return (
          <div key={i} className="flex items-center justify-between text-xs py-2 px-3 bg-slate-50 rounded-xl">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-[#1B3A6B]">{item.label}</span>
            </div>
        ...
        <div key={i} className="flex gap-3 text-xs border-b border-slate-50 pb-3 last:border-0">
          ...
          <div>
            <p className="text-slate-600 leading-relaxed font-medium">
              <span className="font-bold text-[#1B3A6B]">{act.actor_name}:</span> {act.action_description}
            </p>
            <span className="text-[10px] text-slate-400 font-bold">{new Date(act.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8 my-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b pb-4">
        <div>
          <h3 className="text-xl font-black text-[#1B3A6B] uppercase">Active Vacancy Progress Tracker</h3>
          <p className="text-xs text-slate-400 font-bold">11-Stage RSP Workflow — 26 Working Day Target</p>
        </div>
        ...
        {vacancies.map(v => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider whitespace-nowrap transition-all ${
              v.id === currentVac.id ? 'bg-[#1B3A6B] text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
            }`}
          >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-50 p-6 rounded-2xl mb-8">
        <div>
          <h4 className="text-2xl font-black text-[#1B3A6B] uppercase">{currentVac.position_title}</h4>
          <p className="text-xs font-bold text-slate-400 uppercase mt-0.5">{currentVac.assigned_school}</p>
        </div>
        <div className="flex items-center gap-4 mt-4 md:mt-0 shrink-0">
          <span className="text-xs font-black text-slate-500 bg-white px-3 py-1.5 rounded-xl border">
            Applicants: <strong className="text-[#1B3A6B]">{currentVac.total_applicants}</strong>
          </span>
          ...
          {currentVac.workflow.map((node) => {
            ...
            return (
              <div key={node.stage} className="flex flex-col items-center relative z-10 w-20">
                ...
                <span className="text-[9px] font-bold text-slate-400 mt-1">{node.target}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-[#1B3A6B]/5 p-6 rounded-2xl gap-4">
        <span className="text-sm font-black text-[#1B3A6B] uppercase">
