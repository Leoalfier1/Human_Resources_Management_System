    ...
    return (
        <div className="space-y-6 select-none">
            ...
            <div className="flex justify-between items-end">
                <div>
                    <h2 className="text-2xl font-black text-black uppercase tracking-tight italic">Vacancy Posting</h2>
                    <p className="text-xs font-bold text-slate-600 uppercase tracking-widest mt-0.5">Stage 1 of RSP — 10 calendar day application window</p>
                </div>
                {view === 'list' && (
                    <button 
                    ...
                    >
                        ...
                        <form onSubmit={handlePublish} className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-10">
                            ...
                            <div className="space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">Position Title *</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">Item Number *</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">Salary Grade</label>
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">Assigned School / Station *</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">No. of Vacancies</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">Posting Date *</label>
                            ...
                            <div className="space-y-5">
                                <div>
                                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1">Minimum Qualifications *</label>
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-2">Publish To *</label>
                                    <PublishToggles 
    values={formData} 
...
/>
                            ...
                            <div className="lg:col-span-2 border-t pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
                                ...
                                <div className="flex gap-3">
                                    <button 
                                        ...
                                        className="px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest border-2 border-slate-200 text-slate-800 hover:bg-slate-50 transition-all"
                                    >
                                        Cancel
                                    </button>
                                    <button 
                    ...
                    >
                        ...
                        <div className="p-8 border-b border-slate-100 flex justify-between items-center">
                            <h3 className="text-xl font-black text-black uppercase tracking-tight italic">Posted Vacancies</h3>
                            <span className="bg-slate-100 text-slate-800 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em]">
                        ...
                        <div className="overflow-x-auto">
                            {vacancies.length === 0 ? (
                                <div className="p-20 text-center flex flex-col items-center">
                                    ...
                                    <p className="text-slate-600 font-bold uppercase tracking-widest text-sm">No vacancies posted yet</p>
                                </div>
                            ) : (
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-slate-50 text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {vacancies.map((v) => (
                                            <tr key={v.id} className="hover:bg-slate-50/50 transition-colors group">
                                                <td className="px-8 py-5 font-black text-black text-sm">{v.ref_no}</td>
                                                <td className="px-4 py-5 font-black text-black text-sm">{v.position_title}</td>
                                                <td className="px-4 py-5 text-black text-xs font-bold underline opacity-70 cursor-pointer">{v.assigned_school}</td>
                                                <td className="px-4 py-5 font-bold text-slate-800 text-sm">{v.no_of_vacancies}</td>
                                                <td className="px-4 py-5">
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold text-black">{new Date(v.deadline_date).toLocaleDateString()}</span>
                                                        <span className={`text-[10px] font-black uppercase flex items-center gap-1 ${v.days_left < 0 ? 'text-slate-600' : v.days_left <= 4 ? 'text-red-500' : 'text-slate-600'}`}>
                                                            <Clock size={10} /> {v.days_left < 0 ? 'Closed' : `${v.days_left}d left`}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-5">
                                                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full tracking-widest ${v.computed_status === 'active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
                                                        {v.computed_status}
                                                    </span>
                                                </td>
