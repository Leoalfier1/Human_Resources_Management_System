...
export const FileDropzone = ({ file, setFile, error }) => (
  <div className="space-y-2 relative">
    <label className="block text-[10px] font-black text-black uppercase tracking-widest">Division Memorandum (PDF)</label>
    <div className={`border-2 border-dashed rounded-2xl p-6 transition-all flex flex-col items-center justify-center bg-slate-50 ${error ? 'border-red-300' : 'border-slate-200'}`}>
      {!file ? (
        <>
          ...
          <p className="text-xs text-slate-800 font-bold">Drag & drop PDF or <span className="text-black underline cursor-pointer">browse</span></p>
          <input 
            type="file" 
            accept=".pdf" 
        ...
        <div className="flex items-center gap-3 bg-white p-3 rounded-xl shadow-sm border border-slate-200 w-full">
          ...
          <button type="button" onClick={() => setFile(null)} className="text-slate-600 hover:text-red-500 transition-colors">
            <X size={18}/>
          </button>
        </div>
      )}
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all cursor-pointer ${
              isSelected 
                ? 'border-[#1B3A6B] bg-blue-50 text-black shadow-inner' 
