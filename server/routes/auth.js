...
router.post('/register', rateLimit(), async (req, res) => {
    try {
        ...
        if (existingUser.length > 0 && existingUser[0].is_verified) {
            return res.status(409).json({ message: "This email is already registered. Please log in or use another email." });
        }
        ...
        if (existingUser.length > 0) {
            ...
            const [result] = await db.query(
                'INSERT INTO users (full_name, email, mobile, password, is_verified) VALUES (?, ?, ?, ?, 1)',
        ...
        try {
            ...
            return res.status(201).json({ 
                message: "User registered! You may now log in.",
                userId 
            });

        } catch (emailError) {
            console.error("❌ NODEMAILER ERROR:", emailError);
            // If email fails, user is still registered and verified
            return res.status(201).json({ 
...
router.post('/login', rateLimit({ max: 50 }), async (req, res) => {
    try {
        ...
        if (loginType === 'staff' && !staffRoles.includes(user.role)) {
            return res.status(403).json({ 
                message: "Sorry, applicant accounts are only authorized for the Employee Portal." 
            });
        }
        ...
        const token = jwt.sign(
            { id: user.id, role: user.role, applicant_type: user.applicant_type || null, full_name: user.full_name },
