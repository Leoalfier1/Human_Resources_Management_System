...
async function buildAnnexEPayload(applicationId) {
    ...
    if (!['qualified', 'disqualified'].includes(app.status)) {
    ...
    const effectiveVariant = app.letter_type || app.status;
    ...
    if (criteria.length === 0) {
        ...
        criteria = standardLabels.map(sc => ({
            ...
            is_required: true,
            passed: app.status === 'qualified' ? 1 : null,
...
exports.sendAnnexEAdvice = async (req, res) => {
    try {
        ...
        const [userRows] = await db.query(
            'SELECT email FROM users WHERE id = (SELECT user_id FROM applications WHERE id = ? LIMIT 1)', [applicationId]
        );
...
module.exports._buildAnnexEPayload = buildAnnexEPayload;
module.exports._renderAnnexEContent = renderAnnexEContent;
module.exports._generateAnnexEBuffer = generateAnnexEBuffer;
